defmodule Eirinchan.CredentialHash do
  ...
  def hash(value, purpose) when is_binary(value) do
    @prefix <> encoded_digest(value, purpose)
  end

  def fingerprint(value, purpose, length \\ 16) when is_binary(value) and length > 0 do
    encoded_digest(value, purpose) |> binary_part(0, min(length, 43))
  end
  ...
  defp encoded_digest(value, purpose) do
    :crypto.mac(
      :hmac,
      :sha256,
      secret_key_base(),
