defmodule Eirinchan.Stickers do
  @moduledoc """
  Runtime helpers for configurable vichan-style sticker replacements.
  """
  ...
  def entries(%{stickers: entries}) when is_list(entries), do: normalize_entries(entries, [])

  # Preserve compatibility with existing instance configuration while keeping
  # the public release free of instance-specific default sticker content.
  def entries(%{whalestickers: entries}) when is_list(entries), do: normalize_entries(entries, [])

  def entries(settings) when is_map(settings) do
    entries =
      Map.get(settings, :stickers) ||
        Map.get(settings, "stickers") ||
        Map.get(settings, :whalestickers) ||
        Map.get(settings, "whalestickers") ||
        []
  ...
  def update(raw_json) when is_binary(raw_json) do
    with {:ok, decoded} <- Jason.decode(raw_json),
         ...
         config <-
           Settings.current_instance_config()
           |> Map.drop([:whalestickers, "whalestickers"])
           |> Map.put(:stickers, normalized),
